...
class NLPRecommendationAgent(Runnable):
    def __init__(self, project_root):
        ...
        # ── Mental Health ─────────────────────────────────────────────────
        self.mental_model, self.mental_df = MentalHealthDataLoader(project_root).load()
        self.mental_embeddings = self.mental_model.encode(
            self.mental_df['text'].tolist(),
            convert_to_tensor=True,
    ...
    """
    This function gets the preferences from the Mental health services nlp and returns the top 10
    regions to the service through the Coordinator agent.
    """
    def mental_health_invoke(self, signal: MentalHealthNLPSignals) -> MentalHealthRecommenderSignals:
        with torch.no_grad():
            q = self.mental_model.encode(
                signal.preference,
                convert_to_tensor=True,
                normalize_embeddings=True
            )
