...
#
#         # Agents are lazy — only created when first needed.
#         # Do NOT assign self.recommender / self.insight_generator / self.allocation_agent
#         # here — those names are @property and have no setter.
...
#         response = self.allocation_agent.child_invoke(request)
#         return {
#             "total_budget":      response.total_budget,
...
#         df = self.allocation_agent.model.risk_df
#         return {
#             "critical_districts": df[df["Risk_Tier"] == "CRITICAL"]["District"].tolist(),
#             "high_districts":     df[df["Risk_Tier"] == "HIGH"]["District"].tolist(),
#             "moderate_districts": df[df["Risk_Tier"] == "MODERATE"]["District"].tolist(),
