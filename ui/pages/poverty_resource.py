"""
ui/pages/poverty_resource.py
============================
Input  : total budget (float)
Output : allocation table matching the terminal report in the screenshot
"""
...
:root {
    ...
    --accent:   #f0883e; --text:    #e6edf3; --muted:  #8b949e;
    --critical: #ff4d6d; --high:    #fb8500; --moderate:#ffb703; --low:#2dc653;
}
...
.page-header { border-bottom:1px solid var(--border); padding-bottom:1rem; margin-bottom:1.5rem; }
.page-header h1 { font-family:'Syne',sans-serif; font-weight:800; font-size:1.6rem;
    color:var(--text); margin:0; letter-spacing:-0.03em; }
.page-header .sub { font-size:0.8rem; color:var(--muted); }

.control-panel { background:var(--surface); border:1px solid var(--border);
    border-radius:12px; padding:1.4rem 1.2rem; }
.panel-title { font-family:'Syne',sans-serif; font-weight:700; font-size:0.85rem;
    text-transform:uppercase; letter-spacing:0.1em; color:var(--muted); margin-bottom:1.2rem; }
.panel-divider { border:none; border-top:1px solid var(--border); margin:1.1rem 0; }

.tier-box { border-radius:8px; padding:0.7rem 0.9rem; margin-bottom:0.5rem; border:1px solid var(--border); }
...
.badge-CRITICAL { background:rgba(255,77,109,.15); color:var(--critical); border:1px solid rgba(255,77,109,.3); }
.badge-HIGH     { background:rgba(251,133,0,.15);  color:var(--high);     border:1px solid rgba(251,133,0,.3); }
.badge-MODERATE { background:rgba(255,183,3,.15);  color:var(--moderate); border:1px solid rgba(255,183,3,.3); }
.badge-LOW      { background:rgba(45,198,83,.15);  color:var(--low);      border:1px solid rgba(45,198,83,.3); }
...
div[data-testid="stButton"] button[kind="primary"] {
    background:var(--accent)!important; color:#0d1117!important; border:none!important;
...
TIER_COLORS = {"CRITICAL":"#ff4d6d","HIGH":"#fb8500","MODERATE":"#ffb703","LOW":"#2dc653"}
TIER_BG     = {"CRITICAL":"rgba(255,77,109,.07)","HIGH":"rgba(251,133,0,.07)",
               "MODERATE":"rgba(255,183,3,.07)","LOW":"rgba(45,198,83,.07)"}
PB = "rgba(0,0,0,0)"
...
with col_panel:
    ...
    try:
        ...
        for tier, key, color in [
            ("CRITICAL","critical_districts","#ff4d6d"),
            ("HIGH",    "high_districts",    "#fb8500"),
            ("MODERATE","moderate_districts","#ffb703"),
            ("LOW",     "low_districts",     "#2dc653"),
        ]:
...
with col_main:
        ...
        try:
            ...
            if all_d:
                ...
                fig.update_layout(plot_bgcolor=PB, paper_bgcolor=PB,
                    xaxis=dict(showgrid=False, color="#8b949e", tickfont=dict(size=13,color="#e6edf3")),
    ...
    cards += f"""
    <div class="metric-card" style="--card-accent:#f0883e">
    ...
    <div class="metric-card" style="--card-accent:#58a6ff">
    ...
    with ch1:
        ...
        fig_bar.update_layout(plot_bgcolor=PB, paper_bgcolor=PB,
            xaxis=dict(showgrid=False, zeroline=False, showticklabels=False, color="#8b949e"),
            yaxis=dict(showgrid=False, color="#e6edf3", tickfont=dict(size=10)),
    ...
    with ch2:
        ...
        fig_pie.update_layout(plot_bgcolor=PB, paper_bgcolor=PB,
            legend=dict(font=dict(color="#e6edf3", size=10), bgcolor=PB),
    ...
    for i, (_, row) in enumerate(df.iterrows(), 1):
        ...
        <tr>
          ...
          <td style="color:#8b949e">{row['risk_pct']:.2f}%</td>
          <td class="mono">{fmt_rs(row['total_allocation'])}</td>
          <td style="color:#8b949e">{row['allocation_pct']:.2f}%</td>
          <td style="color:#8b949e">{fmt_rs(row['alloc_per_hh'])}</td>
        </tr>"""
