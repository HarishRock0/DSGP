...
def call_recommendations(preference: str) -> dict:
    try:
        res = requests.post(f"{API_BASE}/recommendations", json={"preference": preference}, timeout=30)
        res.raise_for_status()
        return res.json()
    except requests.exceptions.RequestException as e:
        st.error(f"API error (recommendations): {e}")
        return {}

def call_insights(district: str) -> dict:
    try:
        res = requests.post(f"{API_BASE}/insights", json={"district": district}, timeout=30)
        res.raise_for_status()
        return res.json()
    except requests.exceptions.RequestException as e:
        st.error(f"API error (insights): {e}")
        return {}
...
if run_btn:
    if not user_input.strip():
        ...
        with st.spinner("Analysing preferences..."):
            result = call_recommendations(user_input)          # ← API call
...
with st.spinner("Loading insights..."):
    insights = call_insights(selected_district)                # ← API call
